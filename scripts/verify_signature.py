...
def main(argv: list[str]) -> int:
    ...
    print(f"Signed over: {sig.get('signed_payload', default_payload)}")
    print(f"  sha256           : {sha_now}")
    print(f"  chain_head       : {head_now}")
    print(f"  attestation_sha  : {sig.get('attestation_sha', '(absent)')}")
    print(f"  fact_record_hash : {sig.get('fact_record_hash', '(absent)')}")
    ...
    if valid:
        ...
        print("  The signature binds the run-log sha256, the chain head, the")
        print("  deadline-compliance attestation digest, and the input fact-record")
        print("  hash, so it attests this exact ORDERED, COMPLETE run, driven from")
        print("  this exact fact-record, that met these statutory deadlines. A")
        print("  flipped field (sha moves), a reorder/omission (chain head moves), a")
        print("  tampered margin (attestation digest moves), or a changed input")
        print("  (fact-record hash moves) would make this INVALID.")
        print()
        print(f"Note: {DEMO_KEY_CAVEAT}")
        print("=" * 72)
        return 0
