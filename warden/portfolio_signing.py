...
# portfolio receipt carries this, so the two can never be confused.
PORTFOLIO_SIGNED_PAYLOAD = "canonical_json{portfolio_root,run_count}"


def portfolio_payload_bytes(portfolio_root: str, run_count: int) -> bytes:
    """The exact bytes the PORTFOLIO signature is taken over: a small canonical
    JSON object binding the Merkle root over the fleet's chain heads AND the count
    of attested runs.
    ...
    renders as `{"portfolio_root":"...","run_count":N}` with no whitespace. A
    valid signature over these bytes reads as "this exact Merkle root over exactly
    this many sealed runs, attested by this key": change the root or the count and
    the signature no longer verifies."""
    obj = {"portfolio_root": portfolio_root, "run_count": run_count}
...
def sign_portfolio(portfolio_root: str, run_count: int,
    ...
    The signature is taken over the portfolio payload (the Merkle root AND the
    attested run count) under the DISTINCT portfolio label, using the committed
    demo key. The record also carries the manifest digest (so a verifier can
    confirm the canonical manifest it holds is the one signed) and the honest
    ...
    payload = portfolio_payload_bytes(portfolio_root, run_count)
...
def verify_portfolio(portfolio_root: str, run_count: int,
    ...
    """Verify a portfolio signature record against a root and run count.

    True only when the detached signature is valid over the portfolio payload
    (the Merkle root AND the run count) rebuilt from the values passed in, under
    the record's public key. The root and run count are passed by the caller (who
    recomputed them from the sealed runs), NOT read from the record, so a tamper
    that edits the root or count breaks the signature. Returns False on any
    invalid signature or malformed input rather than raising, so a verifier prints
    INVALID and exits nonzero without a stack trace on tampered evidence."""
    payload = portfolio_payload_bytes(portfolio_root, run_count)
    return verify_bytes(
        payload,
