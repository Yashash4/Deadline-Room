...
# file sha == signed sha: the detached signature is taken over the sha256 of
# these exact LF run-log bytes, so the on-disk file hash and the signed hash are
# the same value. (.gitattributes pins these files to LF on every platform.)
_SEALED_SHAS = {
    "normal": (
