...
(any role at all) supplies ONLY a handle() function.

Concept-agnostic by design: nothing in here knows about clocks, filings,
sanctions, regulators, or keys. Windows-safe (no fcntl; per-agent JSONL, no
shared locks). This is the standalone, reusable lifecycle shell: it imports
only the stdlib, `requests`, and the sibling retry helper in this package. It
depends on NO application code.

Verified against the LIVE Band Agent API on 2026-06-13. Authoritative facts
this client encodes:
...
from band_once.retry import call_with_retry, is_transient_status
from band_once.retry import log as net_log
...
class BandAgentShell:
    def __init__(self, api_key: str, agent_name: str,
        ...
        # DEFAULT is 1 (no retry), so an in-process FakeBand-backed test suite is
        # unchanged; a live runner raises it so a transient Band 429/5xx or
        # transport hiccup during a recorded run is retried with backoff instead
        # of killing the run.
    ...
    def post(self, content: str, mentions: Optional[list] = None,
        ...
        recipient. dedup_key, if given, is embedded in the content and checked
        against /context for exactly-once."""
